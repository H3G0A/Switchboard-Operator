...
public class BoardController : MonoBehaviour
{
    ...
    enum City {Madrid, Lebrija, Sevilla, Cadiz, Toledo}
    
    enum Home {Duplex, Flat, Bungalow, Mansion}
    enum CasaEnum {Dúplex, Piso, Bungalow, Mansión };
    string[] Casa = new string[] { "un Dúplex", "un Piso", "un Bungalow", "una Mansión" };

    enum Contract {Rented, Owned}
    enum ContratoEnum { Alquiler, Propiedad}
    string[] Contrato = new string[] { "de Alquiler", "en Propiedad" };

    enum Job {Mechanic = 1, Teacher = 2, Firefighter = 3, Policeperson = 4, Cashier = 5, Baker = 6}
    enum Profesion {Mecánico = 1, Profesor = 2, Bombero = 3, Policía = 4, Cajero = 5, Panadero = 6}
    
    enum MaritalStatus {Single = 0, Engaged = 1, Divorced = 7, Married = 8, Widowed = 9}  
    enum EstadoCivil {Soltero = 0, Prometido = 1, Divorciado = 7, Casado = 8, Viudo = 9}

    enum SocialClass {Lower, Middle, Upper}
    enum ClaseSocial {Baja, Media, Alta}
    

    private void OnEnable() 
    { 
        Application.runInBackground = true; SetLabels(); NextCaller(); 
    }

    private void Start()
    {
        _locale = LocalizationSettings.SelectedLocale.Identifier;
    }
    ...
    private int[] CreateLabelId(string label) 
    { 
        ...
        else if ((digits12 >= 16 && digits12 <= 22) || (digits12 >= 41 && digits12 <= 46) || (digits12 >= 82 && digits12 <= 88)) city = (int)City.Sevilla;
        ...
        bool option1 = (city == (int)City.Sevilla || city == (int)City.Madrid); if (city == (int)City.Toledo)                 home = (int)Home.Flat;
        ...
        int digits34 = int.Parse(label.Substring(2, 2));
        int digits56 = int.Parse(label.Substring(4, 2));
        if ((digits12 > digits34 && digits34 > digits56) || (digits12 < digits34 && digits34 < digits56))
        {
            contract = (home == (int)Home.Flat || home == (int)Home.Bungalow) ? (int)Contract.Rented : (int)Contract.Owned;
        }
        else
        {
            contract = (home == (int)Home.Flat || home == (int)Home.Bungalow) ? (int)Contract.Owned : (int)Contract.Rented;
        }
        ...
        Debug.Log("Trabajo: " + job + "; Estado Civil: " + status + ";");
    ...
    {  
        _enableBoard = false; _roundsToWin--;
        ...
        int[] callerId = _currentCallers[0].labelId;
        int[] receiverId = _currentCallers[1].labelId;
        switch (_locale.Code)
        {
            case ("en"):
                _panelText.text = $"<color=green>City:</color> {(City)callerId[0]}\n<color=green>Home:</color>  {(Home)callerId[1]}\n<color=green>House Contract:</color> {(Contract)callerId[2]}\n<color=green>Job:</color>  {(Job)callerId[3]}\n<color=green>Marital Status:</color> {(MaritalStatus)callerId[4]}\n<color=green>Social Class:</color> {(SocialClass)callerId[5]}";
                _dialogueText.text = $"I'm looking for someone who lives in <color=#EDB974>{(City)receiverId[0]}</color>, in a <color=#EDB974>{(Contract)receiverId[2]}</color> <color=#EDB974>{(Home)receiverId[1]}</color> they are <color=#EDB974>{(MaritalStatus)receiverId[4]}</color>, <color=#EDB974>{(SocialClass)receiverId[5]}</color> class and work as a <color=#EDB974>{(Job)receiverId[3]}</color>.";
                break;


            case ("es"):
                _panelText.text = $"<color=green>Ciudad:</color> {(City)callerId[0]}\n<color=green>Casa:</color>  {(CasaEnum)callerId[1]}\n<color=green>Contrato:</color> {(ContratoEnum)callerId[2]}\n<color=green>Profesión:</color>  {(Profesion)callerId[3]}\n<color=green>Estado Civil:</color> {(EstadoCivil)callerId[4]}\n<color=green>Clase Social:</color> {(ClaseSocial)callerId[5]}";
                _dialogueText.text = $"Busco a alguien que vive en <color=#EDB974>{(City)receiverId[0]}</color>, en <color=#EDB974>{Casa[receiverId[1]]}</color> <color=#EDB974>{Contrato[receiverId[2]]}</color>, está <color=#EDB974>{(EstadoCivil)receiverId[4]}</color>, es de clase <color=#EDB974>{(ClaseSocial)receiverId[5]}</color> y trabaja de <color=#EDB974>{(Profesion)receiverId[3]}</color>.";
                break;
        }
